...
def list_users_route(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while listing users."
        )
...
def create_user_route(
    ...
    try:
        if not validate_password(user_form.hashed_password):
            logger.warning(
                f"Password does not meet criteria for user {user_form.email}"
            )
            raise HTTPException(
                status_code=400, detail="Password does not meet the required criteria."
            )
...
def update_user_route(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while update user."
        )
...
def delete_user_route(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while delete user."
        )
